 ...
 * @LastEditors: zhu-hu
 * @LastEditTime: 2024-03-24 21:18:08
 */
#include "parser.h"
...
void MapParser::publishRouteNetwork() {
  ...
  for (int i = 0; i < map_paths_.size(); i++) {
    ...
    pose.pose.position.x =
        map_nodes_[node_name_to_id_[map_paths_[i].destination_node_name]]
            .x_pos *
        0.001;
    pose.pose.position.y =
        map_nodes_[node_name_to_id_[map_paths_[i].destination_node_name]]
...
void MapParser::publishMapPoint() {
  ...
  for (int i = 0; i < map_nodes_.size(); i++) {
    ...
    point_marker.id = id_count++;
    point_marker.pose.position.x = map_nodes_[i].x_pos * 0.001;
    point_marker.pose.position.y = map_nodes_[i].y_pos * 0.001;
    point_marker.pose.position.z = 0.0;
    map_point_markers.markers.emplace_back(point_marker);
  }
  map_point_pub.publish(map_point_markers);
...
void MapParser::publishPointText() {
  ...
  for (int i = 0; i < map_nodes_.size(); i++) {
    ...
    point_marker.id = id_count++;
    point_marker.pose.position.x = map_nodes_[i].x_pos * 0.001;
    point_marker.pose.position.y = map_nodes_[i].y_pos * 0.001 + 0.6;
    point_marker.pose.position.z = 0.0;
...
void MapParser::drawCenterLine(const MapParser::MAP_PATH &map_path,
  ...
  map_line.color.b = 0.0;
  map_line.color.a = 0.7;

  geometry_msgs::Point temp_point;

  temp_point.x =
      map_nodes_[node_name_to_id_[map_path.source_node_name]].x_pos * 0.001;
  temp_point.y =
      map_nodes_[node_name_to_id_[map_path.source_node_name]].y_pos * 0.001;
...
void MapParser::publishRouteNetwork(const std::vector<int> &route_paths_id) {
  ...
  for (int i = 0; i < route_paths_id.size(); i++) {
    pose.pose.position.x = map_nodes_[route_paths_id[i]].x_pos * 0.001;
    pose.pose.position.y = map_nodes_[route_paths_id[i]].y_pos * 0.001;
    path.poses.push_back(pose);
  }
  path.header.stamp = ros::Time::now();
  path_pub.publish(path);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    path_ids = dijkstra_planner.findShortestPath(map_parser.getGraphOfVertex(),
                                                 source_id, target_id);
    path_found = true;
  ...
  if (path_ids.size() != 0) {
    std::cout << "planning results from [" << source_id << "] to [" << target_id
              << "]" << std::endl;
