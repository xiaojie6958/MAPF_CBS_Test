 ...
 * @LastEditors: zhu-hu
 * @LastEditTime: 2024-03-17 23:22:30
 */
//

#pragma once

#include "path_finder_base.h"
...
public:
  ...
  findShortestPath(std::shared_ptr<std::vector<std::vector<float>>> graph,
                   int source, int target) override;
...
private:
   ...
   */
  std::vector<int> getSolution(const std::vector<int> &parent,
                               const std::vector<float> &dist, int source,
                               int target);
